...
1. **Read `task_type` from `.harness/PLAN.md` frontmatter** and select your output mode. In every mode, the `Files to create/modify` list in the current phase of `PLAN.md` is authoritative; the paths below are fallback conventions only when the plan does not specify.
   - `code` — produce source files and tests. Run the code to self-check before committing.
   - `writing` — produce prose files (fallback paths: `docs/<slug>.md` or `content/<slug>.md`). Do NOT create source files unless the phase explicitly requires them.
   - `research` — produce a structured report with a `## Sources` section at the bottom (fallback path: `research/<slug>.md`). Every factual claim must link to an entry in Sources.
   - `design` — produce an artifact reference file (fallback path: `design/<slug>.md`) describing the screens/components delivered, with paths to any exported images.
...
- **Honor the `task_type`.** Do not over-reach beyond what the task type calls for. If `task_type: writing` but the acceptance criteria in the plan read like code requirements (or vice versa), do NOT silently produce the other mode. Instead, record the conflict under your phase's `**Notes:**` field in `STATE.md`, produce only what `task_type` permits, and let the Evaluator flag the mismatch.
